...
import com.bfgilmer.xcompress.blocks.CompactorMachineBlock;
...
public class CompactorMachineTileEntity extends LockableLootTileEntity implements IHopper, ITickableTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(2, ItemStack.EMPTY);
	private int cooldownTime = -1;
	private long tickedGameTime;
	
	public CompactorMachineTileEntity() {
		super(XcompressTileEntities.COMPACTOR.get());
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container.xcompress.compactor");
	}
		...
		if (this.level != null && !this.level.isClientSide) {
			...
			if (!this.isOnCooldown()) {
				...
				this.tryMoveItems(() -> {
		         ...
		         if (block instanceof CompactorMachineBlock) {
//		        	 CompactorMachineBlock.updateState(blockstate, this.level, this.worldPosition, this.isEmpty());
		         }
			}
	...
	public static boolean suckInItems(IHopper hopper) {
		Boolean ret = HopperBridgeCode.extractHook(hopper);
		if (ret != null)
			return ret;
		IInventory iinventory = getSourceContainer(hopper);
		if (iinventory != null) {
			Direction direction = Direction.DOWN;
			return isEmptyContainer(iinventory, direction) ? false
					: getSlots(iinventory, direction).anyMatch((p_213971_3_) -> {
						return tryTakeInItemFromSlot(hopper, iinventory, p_213971_3_, direction);
					});
		} else {
	...
	private static ItemStack tryMoveInItem(@Nullable IInventory p_174916_0_, IInventory p_174916_1_,
		...
		if (canPlaceItemInContainer(p_174916_1_, p_174916_2_, p_174916_3_, p_174916_4_)) {
			...
			if (flag) {
				if (flag1 && p_174916_1_ instanceof CompactorMachineTileEntity) {
					CompactorMachineTileEntity FlintTileEntity1 = (CompactorMachineTileEntity) p_174916_1_;
					if (!FlintTileEntity1.isOnCustomCooldown()) {
						int k = 0;
						if (p_174916_0_ instanceof CompactorMachineTileEntity) {
							CompactorMachineTileEntity FlintTileEntity = (CompactorMachineTileEntity) p_174916_0_;
	...
	protected Container createMenu(int worldId, PlayerInventory player) {
		return CompactorMachineContainer.createContainer(worldId, player, this);
	}
